...
<div x-data="{ mobileOpen: false, activeDropdown: null, isScrolled: false }" @scroll.window="isScrolled = (window.pageYOffset > 20)"
    class="fixed top-0 inset-x-0 z-50 transition-all duration-500 ease-in-out font-sans text-[#111827]"
    :class="isScrolled ? 'pt-4 px-4 sm:px-6 lg:px-8' : 'pt-0 px-0'">

    <!-- Container Floating -->
    <div class="mx-auto w-full transition-all duration-500 ease-in-out"
        :class="isScrolled
            ?
            'container bg-white/90 backdrop-blur-md shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 rounded-2xl' :
            'max-w-full bg-white border-b border-[#F8FAFC] shadow-sm rounded-none'">

        <!-- Top Bar: Menyusut dan hilang saat di-scroll -->
        @if ($siteSetting->is_topbar_active)
            <div class="hidden md:block w-full text-[12px] font-medium tracking-wide bg-[#047857] text-white transition-all duration-500 ease-in-out origin-top"
                :class="isScrolled ? 'max-h-0 opacity-0' : 'max-h-12 opacity-100'">
                <div class="container mx-auto h-10 flex items-center justify-between px-4 xl:px-8">
                    <!-- Kiri: Info Kontak -->
